...
public class DBUtil {
	...
	public String tableField2ObjProperty(String field){
...
public List<HashMap<String,String>> select (String sql){
	...
	//		System.out.println(tableFields);
			...
			//		String property = tableField2ObjProperty(tfield);
	...
	public  void  insert(Object obj,String table,HashMap<String,String>map) throws SQLException{
		...
		while (entries.hasNext()) {  
		    ...
		    try {
				...
				Method method = cs.getDeclaredMethod("get"+entry.getKey().substring(0, 1).toUpperCase() + entry.getKey().substring(1),null);
...
//			sql += "'"+values[i]+"'";
	...
	public static void main(String[] args) {
		String now = "2018-04-06 11:26";
		String signId="2";
		String sql =  "select * from sign where id="+signId+" and end_time>='"+now+"'";
	/*	String sql = "SELECT class_name,s.begin_time as s_begin_time,s.end_time s_end_time,";
		sql += "(case when s.end_time>'2018-04-05 16:06' then '正在签到' else '已结束' END ) as states ,";
		sql += "(case when id in (SELECT sign_id from student_sign where student_id=4 ) then '签到成功' else '未签到' end) as student_state ";
		sql += " from sign as s ";
		sql += " LEFT JOIN class_room on class_room.class_number=s.class_number ";
		sql += " where s.class_number in (SELECT class_number from student_class where student_id=4);";*/
		System.out.println(new DBUtil().getTableFieldsBySql(sql));
	}
	...
	public  List<FieldInfo> getTableInfo(String table) throws Exception {  
	...
	public String tableType2objType(String type) {
